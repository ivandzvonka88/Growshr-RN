...
const Challengedate = (props) =>{
    ...
    const [end_dateVisibility,Setend_dateVisibility] = useState(false);
    const [end_timeVisibility,Setend_timeVisibility] = useState(false);
    ...
    const Onend_dateFocus = () => {
        Keyboard.dismiss();
        Setend_dateVisibility(true);
    };

    const Onend_timeFocus = () => {
        Keyboard.dismiss();
        Setend_timeVisibility(true);
    };
    ...
    const setStartDateMethod = (event, date) => {
        ...
        if(finaldate == 'Invalid date'){finaldate='Invalid Date'}
        console.log('start_date =',start_date);
        Setstart_date(finaldate);
        Setstart_dateVisibility(false);
    };

    const setEndDateMethod = (event, date) => {
        let day = Moment(date,'DD-MM-YYYY');
        let finaldate = Moment(day).format('DD-MM-YYYY');
        if(finaldate == 'Invalid date'){finaldate='Invalid Date'}
        Setend_date(finaldate);
        console.log('end_date =',end_date)
        Setend_dateVisibility(false);
    };

    const setStartTimeMethod = (event, date) => {
        let day = Moment(date,'HH:mm A');
        let finaldate = Moment(day).format('HH:mm A');
        if(finaldate == 'Invalid date'){finaldate='Invalid Time'}
    ...
    const setEndTimeMethod = (event, date) => {
        let day = Moment(date,'HH:mm A');
        let finaldate = Moment(day).format('HH:mm A');
        if(finaldate == 'Invalid date'){finaldate='Invalid Time'}
        Setend_time(finaldate);
        console.log('end_time =',end_time)
        Setend_timeVisibility(false);
    };


    const OnNextClick =()=>{
        props.navigation.navigate('CreateChallenge4');
    }
    ...
    return(
        ...
        <StatusBar
            ...
            <Text style={{fontSize:myFontSize.body.fontSize,fontFamily:myFontSize.body.fontFamily,lineHeight:myFontSize.body.lineHeight,color:myThemeColors.secondfontcolor}}>Step 3/5</Text>
            </View>
            </View>
            ...
            <View style={{height:hp('8%'),width:wp('100%'),justifyContent:"center",}}>
                <Text
                 ...
                 >
                    Choose Challenge Start/End Date and Time
                </Text>   
            </View>
        ...
        <View style={{height:hp('35%'),width:wp('100%'),}}>
            ...
            <View style={{height:hp('0%'),width:wp('100%'),flexDirection:"row"}}>
                ...
                <View style={{height:hp('8%'),width:wp('44%'),marginLeft:hp('2%'),marginTop:hp('0.5%'),borderColor:"#E1EAF7",borderWidth:hp('0.2%'),borderRadius:hp('1%'),justifyContent:"center",}}>
                    ...
                    <View style={{alignItems:"flex-start",justifyContent:"center",width:wp('29%'),height:hp('5%')}}>
                        ...
                        <TextInput 
                            ...
                            onFocus={Onend_dateFocus}
                            value={end_date}
                        />
                        { end_dateVisibility == true ?
                            <DateTimePicker value={new Date()} mode={'date'}  display="calendar"
                                        onChange={setEndDateMethod}/>
                            :null
                        }
                ...
                <View style={{height:hp('8%'),width:wp('44%'),marginLeft:hp('2%'),marginTop:hp('0.5%'),borderColor:"#E1EAF7",borderWidth:hp('0.2%'),borderRadius:hp('1%'),justifyContent:"center",}}>
                    ...
                    <TextInput 
                        ...
                        onFocus={Onend_timeFocus}
                        value={end_time}
                    />
                    { end_timeVisibility == true ?
                        <DateTimePicker value={new Date()} mode={'time'}  display="clock"
                                    onChange={setEndTimeMethod}/>
                        :null
                    }
